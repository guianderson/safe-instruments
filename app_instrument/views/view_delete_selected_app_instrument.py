...
class AppInstrumentDeleteSelectedView(LoginRequiredMixin, PermissionRequiredMixin, SuccessMessageMixin, DeleteView):
    model = AppInstrument
    template_name = 'app_instrument/delete_selected_app_instrument.html'
    ...
    def get_object(self, queryset=None):
        # Override get_object to get multiple objects based on selected IDs
        return AppInstrument.objects.filter(id__in=self.request.GET.getlist('record_ids'))

    def post(self, request, *args, **kwargs):
        record_ids = self.request.GET.getlist('record_ids')
        if len(record_ids) == 0:
            # Records do not exist, add a message and redirect
            messages.warning(request, 'Nenhum registro selecionado para remoção.')
            return HttpResponseRedirect(self.success_url)

        # Attempt to delete the selected records
        try:
            objects_to_delete = self.get_object()
            deleted_count = 0
            ...
            messages.error(request, "Não foi possível excluir um ou mais itens selecionados porque eles estão sendo referenciados por outros registros.")
            return HttpResponseRedirect(self.success_url)
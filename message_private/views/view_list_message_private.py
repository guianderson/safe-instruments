...
class MessagePrivateListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    model = MessagePrivate
    paginate_by = 15
    template_name = 'message_private/list_message_private.html'
    ...
    def get_queryset(self):
        ...
        queryset = MessagePrivate.objects.filter(recipient_message=self.request.user, flag_read_recipient=False,
                                                 flag_trash_recipient=False,
                                                 flag_soft_excluded_recipient=False).order_by('-updated_at')
        if search:
            queryset = MessagePrivate.objects.filter(recipient_message=self.request.user, flag_read_recipient=False,
                                                     flag_trash_recipient=False, flag_soft_excluded_recipient=False,
                                                     subject__icontains=search).order_by(
                '-updated_at') | MessagePrivate.objects.filter(recipient_message=self.request.user,
                                                               flag_read_recipient=False,
                                                               to_message__first_name=search,
                                                               flag_trash_recipient=False,
                                                               flag_soft_excluded_recipient=False,
                                                               subject__icontains=search).order_by(
                '-updated_at') | MessagePrivate.objects.filter(recipient_message=self.request.user,
                                                               flag_read_recipient=False,
                                                               to_message__last_name=search, flag_trash_recipient=False,
                                                               flag_soft_excluded_recipient=False,
                                                               subject__icontains=search).order_by(
                '-updated_at') | MessagePrivate.objects.filter(recipient_message=self.request.user,
                                                               flag_read_recipient=False,
                                                               to_message__email=search, flag_trash_recipient=False,
                                                               flag_soft_excluded_recipient=False,
                                                               ).order_by('-updated_at')
        return queryset

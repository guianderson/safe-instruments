...
class MessageSendPrivateListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    model = MessagePrivate
    paginate_by = 15
    template_name = 'message_private/list_message_send_private.html'
    ...
    def get_queryset(self):
        ...
        queryset = MessagePrivate.objects.filter(to_message=self.request.user).order_by('-updated_at')
        if search:
            queryset = MessagePrivate.objects.filter(to_message=self.request.user, subject__icontains=search).order_by(
                '-updated_at') | MessagePrivate.objects.filter(to_message=self.request.user,
                                                               first_name__icontains=search).order_by(
                '-updated_at') | MessagePrivate.objects.filter(to_message=self.request.user,
                                                               last_name__icontains=search).order_by('-updated_at')
        return queryset
